...
public class ClientA {
    ...
    public static void main(String[] args) {
        ...
        String directoryPath = Constants.CLIENT2_FILE_ROOT;
                    ...
                    for (int i = 0; i < index; i++) {
                        ...
                        for (int j = 0; j < localFiles.length; j++) {
                            if (files[i].equals(localFiles[j])){
                                ...
                                try {
                                    byte[] fileHash = calculateHash(Constants.CLIENT2_FILE_ROOT + files[i], algorithm);
    ...
    public static void sendFile(String fileName,int senderPort,InetAddress senderIp,InetAddress receiverIP,int receiverPort){
        ...
        packetSender.start();
        FileReader fileReader=new FileReader(bufferMonitor,fileName, Constants.CLIENT2_FILE_ROOT);
        fileReader.start();
        try {
            packetSender.join();
            fileReader.join();				
